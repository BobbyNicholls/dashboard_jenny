...
if age_max < age_min:
    ...
    subset_age = titanic_data[
        (titanic_data["age"] <= age_max) & (age_min <= titanic_data["age"])
    ]
    st.write(
...
subset_fare = titanic_data[
    (titanic_data["fare"] <= fare_max) & (fare_min <= titanic_data["fare"])
]
st.write(
...
"""
TEXT AREAS AND BALLOONS
"""
with st.echo("below"):  # THIS DISPLAYS THE ACTUAL CODE WITHIN THE "with" BLOCK
